...
# Profile DB
CATEGORY_LIST_TABLE = "category_list_v2"
BUDDY_LIST_TABLE = "buddy_list"
PROFILE_INFO_TABLE = "profile_info_v6"
PROF_COL_UID = "1000"
PROF_COL_QID = "1001"
PROF_COL_QQ = "1002"
PROF_COL_GROUP_ID = "25007"
PROF_COL_GROUP_LIST_PB = "25011"
PROF_COL_NICKNAME = "20002"
PROF_COL_REMARK = "20009"
PROF_COL_SIGNATURE = "20011"
# Protobuf in Profile DB
PB_GROUP_ID = "25007"
PB_GROUP_NAME = "25008"
...
PB_IMAGE_IS_FLASH = "45829"
PB_REDPACKET_TYPE = "48412"
PB_REDPACKET_TITLE = "48443"
PB_VOICE_DURATION = "45005"
PB_VOICE_TO_TEXT = "45923"
...
class ProfileManager:
    ...
    def __init__(self, db_path):
        ...
        self.my_uid = ""
        self.user_info = {}  # {uid: {qq, nickname, remark, group_id}}
        self.group_info = {}  # {group_id: group_name}
        self.all_profiles_cache = {}  # 新增，用于缓存所有用户信息

    def load_data(self):
        """
        加载所有用户信息，严格遵循 buddy_list 作为好友关系的唯一来源。
        """
    ...
    def _load_groups(self, cur):
        cur.execute(
            f'SELECT "{PROF_COL_GROUP_LIST_PB}" FROM {CATEGORY_LIST_TABLE} LIMIT 1'
        )
        pb_data = cur.fetchone()
        if not pb_data or not pb_data[0]:
            return

        decoded, _ = blackboxprotobuf.decode_message(pb_data[0])
        group_list_data = decoded.get(PROF_COL_GROUP_LIST_PB)
        if not group_list_data:
            return

        groups = (
            group_list_data if isinstance(group_list_data, list) else [group_list_data]
        )
        for group in groups:
    ...
    def _build_friend_list(self):
        # 此函数现在直接从 self.all_profiles_cache 构建好友列表
        # 需要重新连接数据库以获取buddy_list
        with sqlite3.connect(self.db_path, uri=True) as con:
            cur = con.cursor()
            self.user_info = {}
    ...
    def get_filename(self, uid):
        ...
        return f"{qq}_{safe_name_part}{safe_remark_part}.txt"
...
def _decode_interactive_gray_tip(
    segment: dict, profile_mgr, name_style, name_format
) -> dict or None:
    try:
...
def decode_gray_tip(
    ...
    interactive = _decode_interactive_gray_tip(
        segment, profile_mgr, name_style, name_format
    )
    if interactive:
        return interactive
    if PB_RECALLER_NAME in segment:
        name = segment[PB_RECALLER_NAME].decode("utf-8", "ignore")
        uid_match = re.search(r'uin="(u_\w+)"', name)
        if uid_match:
            display_name = profile_mgr.get_display_name(
                uid_match.group(1), name_style, name_format
            )
            return f"[{display_name} 撤回了一条消息]"
        return f"[{name} 撤回了一条消息]"
    text = segment.get(PB_GRAYTIP_TEXT, b"").decode("utf-8", "ignore")
    if text:
        if any(p.search(text) for p in IGNORE_GRAY_TIP_PATTERNS):
            return None
        return f"[{text}]"
    return "[系统提示]"
...
def decode_message_content(
    content, timestamp, profile_mgr, name_style, name_format, is_timeline=False
) -> list or None:
    if not content:
        return None
    try:
        ...
        for seg in segments:
            ...
            if msg_type == 1:
                part = seg.get(PB_TEXT_CONTENT, b"").decode("utf-8", "ignore")
            elif msg_type == 7:
                ts = seg.get(PB_REPLY_ORIGIN_TS)
                if ts in SALVAGE_CACHE:
                    origin_content = SALVAGE_CACHE[ts]
                else:
                    origin_content = seg.get(PB_REPLY_ORIGIN_SUMMARY_TEXT, b"").decode(
                        "utf-8", "ignore"
                    )
                ...
                sender = profile_mgr.get_display_name(
                    get_placeholder(s_uid), name_style, name_format
                )

                if is_timeline:
                    r_uid = seg.get(PB_REPLY_ORIGIN_RECEIVER_UID, b"").decode("utf-8")
                    receiver = profile_mgr.get_display_name(
                        get_placeholder(r_uid), name_style, name_format
                    )
                ...
                call_type = (
                    "语音通话"
                    if seg.get(PB_CALL_TYPE) == 1
                    else "视频通话" if seg.get(PB_CALL_TYPE) == 2 else "通话"
                )
                ...
                part = decode_gray_tip(seg, profile_mgr, name_style, name_format)
            elif msg_type == 10:
                part = decode_ark_message(seg)
            else:
                part = _parse_single_segment(seg)
            if part:
                parts.append(part)
...
def select_friends(profile_mgr: ProfileManager):
    ...
    for uid, info in profile_mgr.user_info.items():
        if uid == profile_mgr.my_uid:
            continue
        gid = info["group_id"]
        if gid not in friends_by_group:
            friends_by_group[gid] = []
        friends_by_group[gid].append(uid)
    while True:
        print("\n--- 请选择要导出的好友 ---")
        sorted_groups = sorted(
            friends_by_group.items(),
            key=lambda i: profile_mgr.group_info.get(i[0], str(i[0])),
        )
        ...
        to_show = []
        if choice == "a":
            for gid, uids in sorted_groups:
                to_show.extend(uids)
        elif choice in choices:
            to_show = friends_by_group[choices[choice]]
        else:
            print("  -> 无效输入。")
            continue
        print("\n--- 请选择好友 (可多选，用空格或逗号分隔) ---")
        selectable = {str(i + 1): uid for i, uid in enumerate(to_show)}
        for i, uid in enumerate(to_show):
        ...
        selected = [
            selectable[c] for c in re.split(r"[\s,]+", choices_str) if c in selectable
        ]
        if selected:
...
def select_group(profile_mgr: ProfileManager):
    ...
    for uid, info in profile_mgr.user_info.items():
        if uid != profile_mgr.my_uid:
            friends_by_group[info.get("group_id")].append(uid)
...
def process_and_write(
    output_path, rows, profile_mgr, name_style, name_format, is_timeline=False
):
    count = 0
    with open(output_path, "w", encoding="utf-8") as f:
        for row in rows:
            ts, s_uid, p_uid, content = row
            parts = decode_message_content(
                content, ts, profile_mgr, name_style, name_format, is_timeline
            )
            if not parts:
                continue
            ...
            if isinstance(first, dict) and first.get("type") == "interactive_tip":
                ...
                if sender == "N/A":
                    sender = "[系统提示]"
                if is_timeline:
                    if get_placeholder(s_uid) == get_placeholder(p_uid):
                        p_uid = profile_mgr.my_uid
                    receiver = profile_mgr.get_display_name(
                        get_placeholder(p_uid), name_style, name_format
                    )
...
def export_timeline(db_con, config):
    print("\n正在执行“全局时间线”导出...")
    start_ts, end_ts, name_style, name_format, profile_mgr = config.values()
    ...
    path = os.path.join(OUTPUT_DIR, "chat_logs_timeline.txt")
...
def main():
    ...
    if mode == 5:
        ...
        export_user_list(profile_mgr, list_mode)
        print("\n--- 所有任务已完成 ---")
        return
    ...
    if mode == 1:  # 全局时间线
        ...
        targets = [
            uid for uid in profile_mgr.user_info.keys() if uid != profile_mgr.my_uid
        ]
    elif mode == 3:  # 按分组
